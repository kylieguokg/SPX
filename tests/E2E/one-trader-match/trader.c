...
int send_exchange_message(int* order_no, int write_fd, int read_fd){
    ...
    switch(*order_no)
    {
        ...
        // exact match buy order
        case 2:
            sleep(1);
            place_buy_order(1, "MANGO", 10, 10, write_fd);
            sleep(1);
            break;

        // buy order - higher price, take first price
        case 3:
            sleep(1);
            place_buy_order(2, "MILK", 100, 10, write_fd);
            sleep(1);
            break;

        // sell order - lower price
        case 4:
            sleep(1);
            place_sell_order(3, "MILK", 100, 8, write_fd);
            sleep(1);
            break;

        // sell order - higher quantity
        case 5:
            sleep(1);
            place_sell_order(4, "MANGO", 100, 10, write_fd);
            sleep(1);
            break;

        // buy order - lower quantity
        case 6:
            sleep(1);
            place_buy_order(5, "MANGO", 10, 10, write_fd);
            sleep(1);
            break;    
        ...
        case 7:
            sleep(1);
            place_buy_order(6, "MANGO", 100, 10, write_fd);
            sleep(1);
            break;    
